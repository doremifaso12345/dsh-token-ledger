...
import { readZstdLines, ZSTD_AVAILABLE, NODE_REQUIREMENT } from './zstd-frames.js'
...
export class SessionLedger {
  ...
  scan(opts = {}) {
    ...
    // 环境不满足时**直接报出**，而不是逐帧解压失败后把 0 当成结果。
    // 静默的 0 会被误读成「今天没用过」，是最难排查的一种失败。
    if (!ZSTD_AVAILABLE) {
      this._lastScan = {
        generatedAt: now,
        today,
        ...
        breakdown: [],
        sessions: [],
        allTime: { ...ZERO(), totalTokens: 0 },
        sessionCount: 0,
        fileCount: 0,
        stats: { reparsed: 0, reused: 0, badFrames: 0, totalEvents: 0 },
        fatal: {
          code: 'UNSUPPORTED_NODE',
          nodeRequirement: NODE_REQUIREMENT,
          nodeVersion: process.version,
          message:
            `会话日志是多帧 zstd 容器，需要 Node ${NODE_REQUIREMENT} 的 ` +
            `zlib.zstdDecompressSync；当前 ${process.version} 没有该 API。`,
        },
      }
    ...
    // now / today 已在 scan() 入口声明（环境守卫要用到）
