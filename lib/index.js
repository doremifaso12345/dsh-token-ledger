...
import { ZSTD_AVAILABLE, NODE_REQUIREMENT } from './zstd-frames.js'
...
function buildPayload(ledger, days, opts = {}) {
  ...
  return {
    ...
    // 环境不满足时带上原因，让前端能说明「不是没用过，是读不了」
    ...(report.fatal === undefined ? {} : { fatal: report.fatal }),
...
export function apply(ctx, rawConfig) {
  ...
  // 环境守卫：缺少 zstd 解压能力时，逐帧解压会全部失败并被当成坏帧，
  // 最终表现为「用量全是 0」。这里明确报出来，别让人对着 0 排查。
  if (!ZSTD_AVAILABLE) {
    log(
      'error',
      `Node ${NODE_REQUIREMENT} is required (zlib.zstdDecompressSync missing; running ${process.version}). ` +
        `Session logs are multi-frame zstd and cannot be read — usage will be reported as unavailable, not zero.`
    )
  }
          ...
          try {
            ...
            if (sub === '/api/health') {
              sendJson(response, 200, {
                ...
                ttlMs,
                // 环境自检：这两项让「为什么全是 0」一眼可查
                zstdAvailable: ZSTD_AVAILABLE,
                nodeVersion: process.version,
                nodeRequirement: NODE_REQUIREMENT,
              })
              return
            }
